...
class Ajax(object):
    ...
    def __call__(self, request, id):
        ...
        self.ThisRoom = get_object_or_404(Room, pk=testin)
        NewDescription = None
        ...
        # new messages. 
        NewMessages= Message.objects.filter(room=self.ThisRoom, unix_timestamp__gt=self.request_time, event=None)
        #NewMessages = self.ThisRoom.message_set.filter(unix_timestamp__gt=self.request_time, event = None)
        if NewMessages:
            StatusCode = 1
...
class CustomAjax(Ajax):
    def __call__(self, request, id):
        testin=ObjectId(id)
        self.ThisRoom = get_object_or_404(Room, pk=testin)
        participant_list = self.ThisRoom.participants.split(':')
        valid_participant = False
        for participant in participant_list:
            if str(request.user.id) == str(participant):
                valid_participant = True
                break
        if valid_participant:
            return super(CustomAjax, self).__call__(request, id)
        else:
            return HttpResponseBadRequest()

BasicAjaxHandler = CustomAjax()
