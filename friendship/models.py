...
AUTH_USER_MODEL = getattr(settings, 'AUTH_USER_MODEL', 'mongo_auth.MongoUser')
...
class FriendshipRequest(models.Model):
    ...
    #from_user1 = models.ForeignKey(AUTH_USER_MODEL, related_name='friendship_requests_sent')
    #to_user1 = models.ForeignKey(AUTH_USER_MODEL, related_name='friendship_requests_received')

    from_user=models.TextField(models.ForeignKey(AUTH_USER_MODEL, related_name='friendship_requests_sent'))
    to_user=models.TextField(models.ForeignKey(AUTH_USER_MODEL, related_name='friendship_requests_received'))
    ...
    def __unicode__(self):
        return u"User %s friendship requested %s" % (self.from_user, self.to_user)
    ...
    def accept(self):
        ...
        bust_cache('requests', self.to_user)
        bust_cache('sent_requests', self.from_user)
        # Bust reverse requests cache - reverse request might be deleted
        bust_cache('requests', self.from_user)
        bust_cache('sent_requests', self.to_user)
        # Bust friends cache - new friends added
        bust_cache('friends', self.to_user)
        bust_cache('friends', self.from_user)
...
class FriendshipManager(models.Manager,Document):
    """ Friendship manager """

    def friends(self, user):
        """ Return a list of all friends """
        key = cache_key('friends', user)
    ...
    def add_friend(self, from_user, to_user, message=None):
        ...
        bust_cache('requests', to_user)
        bust_cache('sent_requests', from_user)
...
class Friend(models.Model):
    ...
    to_user = models.TextField(models.ForeignKey(AUTH_USER_MODEL, related_name='friends'))
    from_user = models.TextField(models.ForeignKey(AUTH_USER_MODEL, related_name='_unused_friend_relation'))
    ...
    def __unicode__(self):
        return u"User #%s is friends with #%s" % (self.to_user, self.from_user)
